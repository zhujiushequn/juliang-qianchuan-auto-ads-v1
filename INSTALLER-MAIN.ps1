...
try { Start-Transcript -Path $Log -Append | Out-Null } catch { Write-Host "WARN: cannot start transcript: $_" -ForegroundColor Yellow }
...
try {
  ...
  Write-Host ""
  Write-Host "This window should not auto-close." -ForegroundColor Yellow
  Write-Host "If OpenClaw is not on C drive, the installer will detect it or ask for the data folder." -ForegroundColor Yellow
  Write-Host ""

  $Installer = Join-Path $Root "install-windows.ps1"
  if (-not (Test-Path $Installer)) { throw "install-windows.ps1 not found. Please fully extract the zip before running." }
