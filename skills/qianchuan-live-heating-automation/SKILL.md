...
description: 复用“煮酒社群投流龙虾”的巨量千川品牌竞价-直播加热自动化能力。适用于在客户/另一台电脑初始化千川直播加热投流机器人、迁移 SOP/CDP 脚本、检查浏览器登录态、按页面中文原文创建广告组/计划、发布前校验、持久化查重、跨分页监控消耗/商品点击/提价/删除策略。
---

# 千川直播加热自动化技能

目标：把“煮酒社群投流龙虾”的千川 `品牌竞价 > 直播加热` 工作流复用到客户电脑。

## 核心原则

- 只处理 `巨量千川 > 品牌投放 > 品牌竞价 > 直播加热`。
- 默认使用已登录浏览器 + CDP/DevTools；不要用截图坐标。
- MCP/API 只用于读数据和交叉校验；发布、调价、删除等生产写动作必须回到页面，并先通过页面中文字段校验。
- 发布、扣费、授权、验证码、风控、余额/资质/权限问题，必须暂停。
- 未经用户明确授权，不点击 `发布计划`。
- 不删除、不关闭、不修改历史计划或历史广告组；监控删除只允许作用于本轮监控清单内、广告组名称和 ID 都匹配的计划。
...
1. 复制客户包 `workspace-template` 到客户工作区。
...
6. 若出现入口页，按固定入口：`账户 -> 巨量千川 -> 竞价投放 -> 全域投放 -> 目标账户`。
7. 进入 `品牌投放 -> 品牌竞价 -> 直播加热`。
...
- 迁移/安装清单：见 `references/porting-checklist.md`。
