...
6. 确认页面左上已经是 `全域投放` 后，再在中间搜索框输入客户自己的千川账户名，例如 `customer-config.yaml` 里的 `qianchuan.account_name`，按回车。
7. 点击搜索结果里的目标千川账户名称进入。
8. 搜不到时依次搜索完整账户名、品牌名、账户 ID；仍无结果说明当前登录账号可能没有目标账户权限，需要客户确认。
9. 进入后必须核对当前账户名称和账户 ID，不一致就停下，不能创建计划。
10. 然后继续进入 `品牌投放 > 品牌竞价 > 直播加热`。
